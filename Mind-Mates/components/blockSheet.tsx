// components/ChatMenuSheet.tsx
// WhatsApp-style top-right dropdown menu
// Animation: scale + opacity + translateY from top-right origin
// Uses Reanimated for UI-thread animation (zero JS thread, zero frame drops)
...
export const ChatMenuSheet = ({ visible, onClose, items }: Props) => {
  ...
  // ── Animated style ────────────────────────────────────────────
  // TEACHING: WhatsApp dropdown effect has 3 properties animating together:
  //
  //   1. opacity:    0 → 1  (fade in)
  //   2. scaleX/Y:  0.85 → 1  (grows from small to full size)
  //   3. translateY: -8 → 0  (drops down slightly from button)
  //
  // transformOrigin top-right is simulated by:
  //   - anchoring the card to top/right with absolute positioning
  //   - using scaleX + translateX correction so it scales FROM the right edge
  //     not from the center (RN scales from center by default)
  //
  // The math: to scale from RIGHT edge instead of center:
  //   translateX = cardWidth/2 * (1 - scale)
  //   This shifts the card right as it shrinks, making it look like
  //   it's anchored to the top-right corner ✅
  //
