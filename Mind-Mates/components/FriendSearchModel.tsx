...
export const FriendsSearchModal: React.FC<Props> = ({ visible, friends, onClose }) => {
  ...
  useEffect(() => {
    if (visible) {
      setQuery('');
      Animated.parallel([
        Animated.spring(slideAnim, { toValue: 0, useNativeDriver: true, damping: 400, stiffness: 400, mass:0.8 }),
        Animated.timing(fadeAnim,  { toValue: 1, duration: 10, useNativeDriver: true }),
      ]).start(() => { setTimeout(() => inputRef.current?.focus(), 50); });
    } else {
      Keyboard.dismiss();
      Animated.parallel([
        Animated.timing(slideAnim, { toValue: -60, duration: 10, useNativeDriver: true }),
...
const sr = StyleSheet.create({
  ...
  listHeader: { fontSize: 13, color: C.muted, fontWeight: '600', paddingHorizontal: 16, paddingVertical: 10, backgroundColor: C.bg },
  row:        { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12, gap: 12, backgroundColor: C.white },
