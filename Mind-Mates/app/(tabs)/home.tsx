// // app/(tabs)/chat.tsx
// // FIX 1: Realtime channel "cannot add callbacks after subscribe()" ✅
// // FIX 2: handleDelete only hides chat for current user, not other user ✅
// // FIX 3: listKey only changes when ORDER changes (no flash) ✅

// import React, { useCallback, useEffect, useState, useRef } from 'react';
// import {
//   View, Text, FlatList, TouchableOpacity,
//   StyleSheet, StatusBar, RefreshControl, Platform, Alert,
// } from 'react-native';
...
//         <Ionicons name="trash" size={14} color="#f33e3e" />
...
//   btn:   { flexDirection:'row', alignItems:'center', gap:6, backgroundColor:'rgb(255, 245, 245)', paddingHorizontal:14, paddingVertical:8, borderRadius:8 },
//   btnTxt:{ color:'#f33e3e', fontWeight:'700', fontSize:13 },
// });
...
//   const [clearModel, setClearModel] = useState<{ chatId: string; name: string } | null>(null);

//   const listKeyRef    = useRef(0);
//   const friendsRef    = useRef<Friend[]>([]);
//   const allFriendsRef = useRef<Friend[]>([]); // ALL connections including hidden — for search
...
//       // Fetch ALL chats where I'm a participant — hidden_for filtered at APP level below.
//       // RLS only checks participants[], not hidden_for[].
//       // We need ALL chats so that chat_id is available even for hidden chats.
//       // This is critical for: FriendsSearchModal, findChat, chat reappear on new msg.
...
//     // cm = ALL chats keyed by other user's id (includes hidden)
...
//       const p  = pm[otherId];
//       const ch = cm[otherId];
//       // isHidden: this chat exists but is in user's hidden_for list
//       // We still include it in the data so chat_id is preserved everywhere.
//       // The LIST query below filters it out; but it remains available for
//       // FriendsSearchModal and findChat lookups.
...
//       // visible = non-hidden chats shown in the list
//       const visible  = allFresh.filter((f: any) => !f.is_hidden);
//       // allFriendsRef = all connections (hidden + visible) for search & chat lookup
...
//   // ── FIX 1: Realtime — definitive crash fix ───────────────────
//   // TEACHING: The crash "cannot add postgres_changes after subscribe()"
//   // happens because supabase.channel(SAME_NAME) returns the SAME
//   // cached instance. If that instance is already subscribed,
//   // calling .on() on it crashes.
//   //
//   // React Strict Mode runs effects TWICE in development:
//   //   Run 1: create channel → subscribe ✅
//   //   Cleanup: removeChannel (async — not instant)
//   //   Run 2: supabase.channel(same name) → returns OLD instance
//   //          OLD instance still subscribed → .on() crashes ❌
//   //
//   // DEFINITIVE FIX: Use unique channel name with Date.now()
//   //   Every call creates a brand-new channel instance
//   //   Never reuses a stale subscribed instance ✅
//   //   The cleanup removes it by reference (not by name) ✅
//   useEffect(() => {
//     if (!user?.id) return;
//     const uid = user.id;

//     // Unique name = guarantees new instance every time ✅
//     // Even if old cleanup is still in-flight, this won't conflict
//     const topic = `cl-${uid}-${Date.now()}`;

//     // Build channel — ALL .on() calls must be before .subscribe()
//     const ch = supabase
//       .channel(topic)
...
//         if (isNowHidden) {
//           // Chat was just hidden for me (e.g. I just cleared/hid it)
...
//         // Chat is visible (either always was, or just became visible again)
//         // If it was hidden before and now hidden_for=[] (send_message cleared it),
//         // the chat needs to reappear → call loadFriends() if not in list
//         setAndSort(prev => {
//           const idx = prev.findIndex(f => f.user_id === otherId);
//           if (idx === -1) {
//             // Not in visible list — might be a hidden chat reappearing
//             // loadFriends will refetch and add it back
//             loadFriends();
//             return prev;
//           }
//           const next = [...prev];
...
//     // Cleanup removes THIS specific instance by reference ✅
//     return () => {
//       supabase.removeChannel(ch);
//       channelRef.current = null;
//     };
//   }, [user?.id]); // Only re-run when uid changes, not on every render

//   // ── Hide chat — Instagram soft delete (per-user only) ────────
//   // TEACHING: We add uid to chat.hidden_for[] — not delete the row.
//   //   Other user sees ZERO change ✅
//   //   If they send a new message → send_message clears hidden_for[]
//   //   → chat reappears in your list automatically ✅
//   //   This is exactly how Instagram DM delete works.
//   const handleDelete = useCallback(async (f: Friend) => {
...
//     if (!f.chat_id) return; // no chat yet

//     try {
//       // 2. Soft hide — adds uid to hidden_for[], other user unaffected ✅
...
//     await Promise.all(targets.map(f => handleDelete(f)));
//   }, [swipedIds, friends, handleDelete, closeAllExcept]);

//   // ── Clear chat — soft clear for current user only ───────────────
//   // TEACHING: clear_chat marks all messages deleted_for[uid]
//   //   + adds uid to chat.hidden_for[]
//   //   Messages NOT deleted from DB → other user unaffected ✅
//   //   On refresh: chat filtered by hidden_for → stays hidden ✅
//   //   New message from other user → send_message clears hidden_for → reappears ✅
//   const handleClear = useCallback((f: Friend) => {
//     if (!f.chat_id) return;
//     Alert.alert('Clear Chat', `Clear chat with ${f.full_name}?\n\nThis only clears it for you.`, [
...
//       <Ionicons name="search-outline" size={20} color={C.text} style={{ right:9 }} />
//     </TouchableOpacity>
//     <TouchableOpacity onPress={onMenuPress} style={{ padding:8 }}>
//       <Ionicons name="ellipsis-vertical" size={20} color={C.text} style={{ left:11 }} />
//     </TouchableOpacity>
//   </View>
// );
...
//   header:      { flexDirection:'row', alignItems:'center', paddingHorizontal:20, paddingVertical:15, backgroundColor:C.white, top:5 },
...
// FIX 2: handleDelete only hides chat for current user, not other user ✅
// FIX 3: listKey only changes when ORDER changes (no flash) ✅
// FIX 4: All Alert.alert replaced with ConfirmModal ✅
