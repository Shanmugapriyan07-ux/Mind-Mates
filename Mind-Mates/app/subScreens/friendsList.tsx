...
export default function ConnectionsListScreen() {
  ...
  // FIX 1: Determine target user
  // TEACHING: Three cases:
  //   A) No params.userId → viewing MY connections → use me?.id
  //   B) params.userId === me?.id → also MY connections
  //   C) params.userId is someone else → THEIR connections (read-only)
  //
  // BUG WAS: params.userId could be '' (empty string passed from profile)
  //   ''.trim() = '' → || me?.id → shows MY connections even for case C ❌
  //
  // FIX: Use params.userId if it's a non-empty string, else fall back to me
