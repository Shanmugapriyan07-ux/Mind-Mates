...
export default function ProfileImageScreen() {
  ...
  const handleAvatarPressIn = () =>
    Animated.spring(avatarScale, { toValue: 0.94, useNativeDriver: true, speed: 50, bounciness: 8 }).start();
  ...
  const openSheet = () => {
    setShowPicker(true);
    Animated.spring(sheetAnim, { toValue: 1, useNativeDriver: true, damping: 280, stiffness: 180 }).start();
  };
  const closeSheet = () => {
    Animated.timing(sheetAnim, { toValue: 0, duration: 100, useNativeDriver: true })
  ...
  const handleNext = useCallback(async () => {
    ...
    if (!imageUri || imageUri === profile?.profileImage) {
      router.push('/(profileSetUp)/SkillSelect'); return;
    }
    setSaving(true);
    try {
  ...
  return (
    <SafeAreaView style={st.container}>
      <View style={st.header}>
        ...
        <Text style={st.subtitle}>Help others recognize you. You can change this any time.</Text>
      </View>
      ...
      <View style={st.avatarSection}>
        <Animated.View style={{ transform: [{ scale: avatarScale }] }}>
          ...
          >
            {imageUri
              ...
              : (
                <View style={[st.initialsCircle, avatarStyle]}>
                  <Text style={[st.initialsText, { fontSize: AVATAR_SIZE * 0.32 }]}>
                    {initials}
                  </Text>
                </View>
              )
            ...
            {uploading && (
              <View style={[st.uploadOverlay, { borderRadius: AVATAR_SIZE / 2 }]}>
          ...
          {saving
            ...
            : (
              <Text style={st.nextBtnText}>
                {imageUri && imageUri !== profile?.profileImage ? 'Save & Continue' : 'Continue'}
              </Text>
            )
          }
        </TouchableOpacity>

        <TouchableOpacity style={st.skipBtn} onPress={handleSkip} disabled={isBusy}>
        ...
        >
          ...
          {imageUri && (
            <TouchableOpacity style={st.sheetOption} onPress={removePhoto}>
              ...
              <Text style={[st.sheetOptionText, { color: '#E53935' }]}>Remove photo</Text>
            </TouchableOpacity>
          )}
