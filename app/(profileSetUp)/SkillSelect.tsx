...
const SKILLS: Skill[] = [
  ...
  { id: 90, name: "React Native",    category: "Tech",     icon: "phone-portrait-outline" },
...
const SkillCard = React.memo(
  ({
    ...
    return (
      ...
      >
        <Animated.View
          style={{ transform: [{ scale }], opacity: isDisabled ? 0.4 : 1 }}
        >
          {isSelected ? (
            <LinearGradient
  ...
  const toggleSkill = useCallback((id: number) => {
    setSelectedIds((prev: Set<number>) => {
      ...
      if (next.has(id)) {
        ...
        setShowLimit(true);
        setTimeout(() => setShowLimit(false), 2500);
      }
      return next;
    });
  }, []);
      ...
      <View style={st.bottomArea}>
        {showLimit && (
          <View style={st.limitWarn}>
            <Text style={st.limitWarnText}>
              ⚠️ Max 3 skills. Deselect one to choose another.
            </Text>
          </View>
        )}
...
const st = StyleSheet.create({
  ...
  tabs: { marginBottom: 5, paddingVertical: 4, width: "100%" },
