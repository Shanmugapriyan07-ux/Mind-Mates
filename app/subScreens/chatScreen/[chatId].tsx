...
import {
  VoiceMessageBubble,
  VoiceStatus,
} from "@/components/voiceMessageBubble";
...
const extractMediaUrl = (m: string) =>
...
const IMAGE_SIZE_CACHE_MAX = 200;
...
function cacheImageSize(
  uri: string,
  val: { width: number; height: number; ratio: number },
) {
  if (IMAGE_SIZE_CACHE.size >= IMAGE_SIZE_CACHE_MAX) {
    const oldestKey = IMAGE_SIZE_CACHE.keys().next().value;
    if (oldestKey !== undefined) IMAGE_SIZE_CACHE.delete(oldestKey);
  }
  IMAGE_SIZE_CACHE.set(uri, val);
}
const secToMs = (ts: number) => (ts < 10_000_000_000 ? ts * 1000 : ts);
const formatTime = (ts: number) =>
  new Date(secToMs(ts)).toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  });
...
const StatusTick = ({
  ...
  if (failed)
    return <Ionicons name="alert-circle-outline" size={s(14)} color={C.red} />;
  if (pending)
    return <ActivityIndicator size={s(10)} color="rgba(255,255,255,0.7)" />;
  if (status === "seen") return null;
  return (
    <Ionicons name="checkmark" size={s(16)} color="rgba(255,255,255,0.7)" />
  );
};
...
const ReplyQuote = ({
  ...
  const preview = isMedia
    ? caption || (isVid ? " Video" : " Photo")
    : replyToText;
  return (
    <View style={t.replyQuote}>
      <Text style={t.replyQuoteName}>
        {replyToSender === myId ? "You" : otherName}
      </Text>
      <View style={{ flexDirection: "row", alignItems: "center", gap: s(6) }}>
        ...
        <Text style={t.replyQuoteText} numberOfLines={1}>
          {preview}
        </Text>
      </View>
    </View>
...
const ReactionsRow = ({
  ...
  onReact: (m: any, e: string) => void;
  msg: any;
}) => {
  let rx: { userId: string; emoji: string }[] = [];
  try {
    rx = JSON.parse(reactionsJson);
  } catch {}
...
const MediaBubble = React.memo(
  ({
    ...
    const {
      ...
      loading,
      error: sizeError,
    } = useRemoteImageSize(isVid ? null : url);
    ...
    return (
      <View style={t.mediaBubbleWrap}>
        <TouchableOpacity
          onPress={onPress}
          activeOpacity={0.88}
          disabled={pending || (hasError && !isVid) || !url}
        >
          <View
            style={[
              t.mediaContainer,
          ...
          >
            ...
            {(loading || pending) && (
              <View
                style={[StyleSheet.absoluteFillObject, t.mediaLoadingOverlay]}
              >
        ...
        {!!caption && (
          <Text style={[t.mediaCaption, isMe ? t.myText : t.otherText]}>
            {caption}
          </Text>
        )}
      </View>
    ...
    if (!panRef.current) {
      panRef.current = PanResponder.create({
        ...
        onPanResponderMove: (_, g) => {
          swipeX.setValue(
            isMe
              ? Math.max(-MAX, Math.min(0, g.dx))
              : Math.min(MAX, Math.max(0, g.dx)),
          );
        },
        ...
        >
          ...
          >
            ...
            {isMediaMsg(item.message) ? (
              ...
              <Text style={[t.msgText, isMe ? t.myText : t.otherText]}>
                {item.message}
              </Text>
            )}
            {(item as any).edited && (
  ...
  ({
    ...
    return (
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          <Text
            style={[ch.headerName, iBlockedThem && { color: "#fff" }]}
            numberOfLines={1}
          >
            {name ?? "Chat"}
          </Text>
          {iBlockedThem ? (
            <Text
              style={[ch.headerStatus, { color: "#EF4444", fontWeight: "600" }]}
            >
              USER BLOCKED
            </Text>
          ) : isOtherTyping ? (
...
export default function ChatScreen() {
  ...
  useEffect(() => {
    ...
    if (unreadFingerprint && unreadFingerprint !== lastMarkedRef.current) {
      lastMarkedRef.current = unreadFingerprint;
      const uid = user.id;
      ...
      markReadRef.current = setTimeout(() => {
        callFn({ action: "mark_chat_read", chatId }).catch((_err: any) => {
  ...
  }, [chatId, user?.id, unreadFingerprint, messages, setMessages]);
  useEffect(() => {
    return () => {
      if (markReadRef.current) clearTimeout(markReadRef.current);
    };
  ...
  const handleVoiceOptimistic = useCallback(
    (tempId: string, durationMs: number, waveform: number[]) => {
  ...
  const handleVoiceSuccess = useCallback(
    (tempId: string, audioUrl: string, messageId: string) => {
      setMessages((prev: any) =>
        prev.map((m: any) =>
          m.$id === tempId
    ...
    async (msg: ChatMessage, emoji: string) => {
      ...
      setMessages((prev: any) =>
        prev.map((m: any) => {
          ...
          let rx: { userId: string; emoji: string }[] = [];
          try {
            rx = JSON.parse((m as any).reactions || "[]");
          } catch {}
  ...
  const handleEdit = useCallback((msg: ActionMessage) => {
    setEditingMsg(msg);
    chatInputRef.current?.setText(msg.message);
    setReplyTo(null);
    setTimeout(() => chatInputRef.current?.focus(), 100);
  }, []);
  ...
  const handleDelete = useCallback(
    (msg: ActionMessage) => {
      ...
      setDeleteModal({
        msg,
        mode: isMine && ageMs < 60_000 ? "choose" : "soft",
      });
    },
    [user?.id],
  );

  const doSoftDelete = useCallback(
    (msg: ActionMessage) => {
      setMessages((prev: any) => prev.filter((m: any) => m.$id !== msg.$id));
      withRetry(() =>
        callFn({ action: "delete_message", messageId: msg.$id }),
      ).catch(() => {});
    },
    [setMessages],
  );

  const doHardDelete = useCallback(
    (msg: ActionMessage) => {
      setMessages((prev: any) => prev.filter((m: any) => m.$id !== msg.$id));
      withRetry(() =>
        callFn({ action: "delete_for_everyone", messageId: msg.$id }),
      ).catch(() => {});
    },
    [setMessages],
  );
  ...
  const handleClearChat = useCallback(() => {
    if (chatId) setClearChatModal(true);
  }, [chatId]);
  const doClearChat = useCallback(async () => {
    ...
    try {
      ...
      console.warn("clear_chat failed:", e?.message);
    }
  }, [chatId, setMessages]);
    ...
    async (caption: string) => {
      ...
      try {
        ...
        if (pendingMedia.type === "image")
          uploadUri = await compressForUpload(pendingMedia.uri, "chat");
  ...
  const handleSend = useCallback(
    async (text: string) => {
      ...
      if (editingMsg) {
        ...
        setSending(true);
        const prevText = editingMsg.message;
        setEditingMsg(null);
        chatInputRef.current?.clear();
        setMessages((prev: any) =>
          prev.map((m: any) =>
        ...
        callFn({
          ...
          setMessages((prev: any) =>
            prev.map((m: any) =>
              m.$id === msgId ? { ...m, message: prevText } : m,
            ),
          );
        });
        setSending(false);
  ...
  const handleOpenMenu = useCallback(() => setMenuSheet(true), []);
  const handleOpenMediaViewer = useCallback(
    (uri: string, type: "image" | "video") => setViewerMedia({ uri, type }),
    [],
  );
  const handleCloseMediaViewer = useCallback(() => setViewerMedia(null), []);
  const handleActionMsgFromBubble = useCallback((msg: ChatMessage) => {
    setActionMsg({
      $id: msg.$id,
      sender_id: msg.senderId,
      message: msg.message,
      ...
      return (
        <View key={item.$id}>
          ...
          <MessageBubble
            ...
            onLongPress={handleActionMsgFromBubble}
            onReact={handleReact}
            onReply={handleReply}
  ...
  return (
    <SafeAreaView style={ch.safe} edges={["top", "bottom"]}>
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <View style={ch.emptyChat}>
                  <Text style={ch.emptyChatText}>
                    Say hello to {resolvedName}!
                  </Text>
                </View>
              }
      ...
      />
      <MediaPreview
        uri={pendingMedia?.uri ?? null}
        type={pendingMedia?.type ?? "image"}
        onSend={handleMediaConfirm}
      ...
      <MessageActionSheet
        ...
        onReact={(msg, emoji) =>
          handleReact(msg as unknown as ChatMessage, emoji)
        }
      ...
      <ConfirmModal
        ...
        message="Remove this message just for you, or delete it for everyone?"
