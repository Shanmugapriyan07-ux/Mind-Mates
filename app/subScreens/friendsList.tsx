      ...
      <View style={sl.cardWrapper}>
        ...
        <Animated.View style={{ transform: [{ translateX: hintX }] }}>
          <Swipeable
            ...
            renderLeftActions={
              isMyProfile && onRemove
                ? () => <RemoveAction onPress={handleRemove} />
                : undefined
            }
          >
            <TouchableOpacity
            ...
            >
              <View style={sl.row}>
                ...
                />
                <View style={sl.info}>
                  <Text style={sl.name} numberOfLines={1}>
                    {item.full_name}
                  </Text>
                  {!!item.location && (
                    <View style={sl.locRow}>
                      <Ionicons
                        ...
                        color={C.muted}
                      />
                      <Text style={sl.locText} numberOfLines={1}>
                        {item.location}
                      </Text>
                    </View>
                  )}
                  {skills.length > 0 && (
                    <Text style={sl.skills} numberOfLines={1}>
                      {skills.join(" · ")}
                    </Text>
                  )}
                </View>
...
export default function ConnectionsListScreen() {
    ...
    async (isRefresh = false) => {
      ...
      try {
        ...
        const pairs: { otherId: string; connId: string }[] = [
          ...(sentRes.data ?? []).map((d: any) => ({
            otherId: d.receiver_id,
            connId: d.id,
          })),
          ...(recvRes.data ?? []).map((d: any) => ({
            otherId: d.sender_id,
            connId: d.id,
          })),
        ].filter((p) => p.otherId);
  ...
  const handleRemove = useCallback(
    async (item: ConnectedUser) => {
      setConnections((prev) =>
        prev.filter((c) => c.connection_id !== item.connection_id),
      );
      try {
        await callFn({
        ...
        console.warn("remove_friend failed:", e?.message);
        fetchConnections();
      }
    },
    [fetchConnections],
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: ConnectedUser; index: number }) => (
      <ConnectionCard
        item={item}
        ...
        ListEmptyComponent={
          <View style={sl.empty}>
            ...
            <Text style={sl.emptyTitle}>
              {isMyProfile
                ? "No Mindmates yet"
                : `${ownerName} has no Mindmates yet`}
            </Text>
...
ConnectionsListScreen.whyDidYouRender = true;
