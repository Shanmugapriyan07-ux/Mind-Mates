  ...
  return (
    ...
    >
      <Pressable style={sh.backdrop} onPress={onClose}>
        <Pressable
          style={[sh.card, { paddingBottom: insets.bottom + 14 }]}
          onPress={() => {}}
        >
          ...
          >
            ...
            <Text style={[sh.rowText, { color: C.text }]}>
              Delete Notification
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
              ...
              <View style={s.info}>
                ...
                {!!skillDots && (
                  <View style={s.skillsRow}>
                    <Text style={s.skillsText} numberOfLines={1}>
                      {skillDots}
                    </Text>
                    {extra > 0 && (
...
const BulkDeleteBar = ({
  ...
  const hasMinCount = count >= 2;
  useEffect(() => {
    Animated.spring(anim, {
      toValue: hasMinCount ? 1 : 0,
      useNativeDriver: true,
      damping: 140,
      stiffness: 350,
    }).start();
...
export default function NotificationsScreen() {
  ...
  useEffect(() => {
    ...
    const unreadToMark = notifs.filter(
      (n: NotifItem) => !n.is_read && n.type !== "connection_request",
    );
    if (!unreadToMark.length) return;
    setNotifs((p: NotifItem[]) =>
      p.map((n: NotifItem) =>
        unreadToMark.some((um) => um.id === n.id) ? { ...n, is_read: true } : n,
      ),
    );
    Promise.all(
  ...
  const handleSwipeOpen = useCallback(
    (id: string) => setSwipedIds((prev) => new Set([...prev, id])),
    [],
  );
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    const failed = results.reduce<NotifItem[]>((acc, res, i) => {
      if (res.error) {
        console.warn(" Bulk delete failed for", ids[i]);
        acc.push(toDelete[i]);
      }
      return acc;
    }, []);
  ...
  const handleAccept = useCallback(
    async (item: NotifItem) => {
      ...
      setNotifs((p: any) =>
        p.map((n: any) => (n.id === item.id ? { ...n, type: "accepted" } : n)),
      );
      ...
      try {
        ...
        setNotifs((p: any) =>
          p.map((n: any) =>
            n.id === item.id ? { ...n, type: "connection_request" } : n,
          ),
        );
  ...
  const handleReject = useCallback(
    async (item: NotifItem) => {
      ...
      rejectRequest(item.connection_id, item.id, item.sender_id).catch(() =>
        loadNotifs(),
      );
    },
        ...
        ListEmptyComponent={
          <View style={s.empty}>
            ...
            <Text style={s.emptySub}>
              When someone wants to connect, it&apos;ll appear here.
            </Text>
          </View>
        }
      />
...
NotificationsScreen.whyDidYouRender = true;
