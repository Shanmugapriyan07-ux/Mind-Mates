      ...
      {showPicker ? (
        <Animated.View style={[sh.picker, { opacity: fadeAnim }]}>
          ...
          <FlashList
            ...
            contentContainerStyle={sh.catScroll}
            renderItem={({ item, index }) => (
              <TouchableOpacity
                style={[sh.catBtn, activeCategory === index && sh.catBtnActive]}
                onPress={() => setCategory(index)}
              >
                <Text style={sh.catEmoji}>{item.emojis[0]}</Text>
              </TouchableOpacity>
            )}
          />
          <Text style={sh.catLabel}>
...
const Row = ({
  ...
  <TouchableOpacity style={sh.row} onPress={onPress} activeOpacity={0.7}>
    <Text style={[sh.rowLabel, labelColor ? { color: labelColor } : {}]}>
      {label}
    </Text>
