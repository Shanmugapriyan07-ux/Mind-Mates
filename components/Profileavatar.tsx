...
// ─── Memory cache ─────────────────────────────────────────────────────────────
// Tracks URIs we've already loaded this session
// Skips image flicker on re-renders / screen revisits
...
export const ProfileAvatar: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    // Local URI or already seen — show instantly
    if (
      loadedUris.has(uri)        ||
      uri.startsWith('file://')  ||
      uri.startsWith('data:')    ||
  ...
  const onLoad = () => {
    if (uri) loadedUris.add(uri);   // remember for next render
    setReady(true);
  ...
  return (
    <View style={[
      ...
      {/* Initials — always visible behind image while loading */}
      {!!name && (
      ...
      {/* Image — React Native handles disk caching automatically */}
      {!!uri && (
        <Animated.Image
          source={{
            uri,
            // Tell RN to cache aggressively
...
// ─── Call this when user changes their photo ──────────────────────────────────
